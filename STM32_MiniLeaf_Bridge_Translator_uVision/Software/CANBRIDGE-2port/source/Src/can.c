...
void MX_CAN1_Init(void)
{
  ...
  hcan1.Instance = CAN1;
  hcan1.Init.Prescaler = 4; // set to 4 for 500k buad OR 20 for 100k buad, this is typicaly (4) 500K for the mini-leaf bridge
...
{
    // Re-entrancy guard: prevent ISR and main loop from executing simultaneously
    static volatile uint8_t sending[2] = {0};
    ...
    {
        if( HAL_CAN_GetTxMailboxesFreeLevel( CANChanRef[ channel ] ) >= 1 )
        {
