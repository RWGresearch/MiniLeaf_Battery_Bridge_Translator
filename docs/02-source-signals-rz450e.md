...
| `0x4A7` | `temp_max`, `temp_min`, + probe # | raw already °C (2026-08-09: this project stores/displays all temps in °C, the old ×1.8+32 °F step no longer applies) | Pack-level temperature extremes. |
...
| `0x4AA` | `temp_01`..`temp_16` (muxed) | raw already °C, whole-degree resolution | All 16 temperature probes. **As of 2026-08-14 this is the BACKUP source only** (`temp_NN_can`) — DID `0x1814` below is now PRIMARY for these same 16 signals, see the DID table's own note and `06-realtime-engine-and-watchdog.md`. |
...
| **16 temperatures** (full snapshot) | DID `0x1814`: `03 22 18 14` | `uint16@(3+(N-1)*2)/256.0 − 50.0` → °C (this project stores °C, so the reference project's further ×9/5+32 °F step is dropped — see `bridge/rz450e_signals.py`'s `decode_temp_probes_did()`) | **As of 2026-08-14 (user directive) this is the PRIMARY source for `temp_01`..`temp_16`** — real 1/256°C resolution vs. `0x4AA`'s whole-degree quantization, polled on its own ~10s gate (`did_temp_poll_interval_s`, GUI-editable via `state.engine_timing` — "Timing" tab, not folded into the SoC/capacity/primary-V-I round-robin above). `0x4AA` is the BACKUP, used whenever the DID reading is older than `did_temp_fresh_window_s` (20s default, provisional — see `docs/11`). This inverts the general "prefer fast broadcast over DID" guidance at the top of this section, specifically for temp probes only, because DID's resolution advantage here outweighs its slower cadence (temperature changes slowly). `temp_probe_cross_check` (`05-battery-management-safety.md`) compares the two sources directly, per probe. |
