...
Track per-source-group freshness using signals the RZ450e project already confirmed are usable for
exactly this purpose (`02-source-signals-rz450e.md`):
- `0x358` and `0x3F1` rolling `alive_counter`s (the latter 4-bit, wraps every 16 frames)
- `0x424` 5-second tick counter
- Toyota's additive checksum, present on 10 of the 12 confirmed messages — a checksum that stops
  validating (even if frames keep arriving) is itself a staleness/corruption signal

A counter that stops incrementing, or a checksum that stops matching, means the underlying data is
stale even if CAN frames are technically still arriving on the bus.

**Two-stage escalation** (per user instruction):
1. At **60 seconds** stale for a required input group → **soft cut** (`capacity_empty`). Most
   transient CAN hiccups should resolve well before this.
...
This watchdog logic is independent per source-group (fast raw-CAN group vs. DID/PID group), since
they have very different native update rates and a DID poll being "due" isn't itself a fault the
way a stopped alive-counter is.
...
Every soft cut, hard cut, and monitor warning in `bridge/management_engine.py`'s curated features
already **auto-clears** the moment its triggering reading recovers — kept as the default behavior,
matching the real Leaf's own "the condition resolves and the dash error goes away" pattern (a
genuinely different, bigger decision — whether hard cuts should instead *latch* until a real
power-cycle/manual acknowledgment — is tracked as an open item, `12-nmc-bms-design-research.md`'s
finding F8, and deliberately not changed here).
...
every HVBAT ID, idle vs. real charge-session captures) and gated behind an opt-in GUI checkbox
(`gui/panels.py`'s `ChargeEmulationPanel`, the "Charge Emulation" tab) — see `03-target-signals-
leaf.md` for the field-level description.
...
say over the ramped `charger_limit_kw` value, never bypassed. It requires **both**:
